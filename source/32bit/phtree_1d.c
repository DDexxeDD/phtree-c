...
typedef struct ph1_node_t
{
	...
	ph1_node_t* parent;
	// children are either ph1_node_t* or ph1_entry_t*
	// if the node is a leaf
	// 	children are ph1_entry_t*
	// if the node is _not_ a leaf
	// 	children are ph1_node_t*
	void* children[PHTREE_KEY_ONE << 1];
...
typedef struct ph1_window_query_t
{
	ph1_point_t min;
	ph1_point_t max;
	/*
...
static void point_subtract (ph1_point_t* point_a, unsigned int amount)
{
	for (int dimension = 0; dimension < DIMENSIONS; dimension++)
	{
...
static void point_add (ph1_point_t* point_a, unsigned int amount)
{
	for (int dimension = 0; dimension < DIMENSIONS; dimension++)
	{
		// prevent integer wrapping
...
static bool node_in_window (ph1_node_t* node, ph1_window_query_t* window)
{
...
static bool entry_in_window (ph1_entry_t* entry, ph1_window_query_t* window)
{
...
static void node_add_entry (ph1_node_t* node, ph1_point_t* point)
{
	...
	// 	create a new entry
	ph1_entry_t* new_entry = phtree_calloc (1, sizeof (*new_entry));
 ...
 * create a new node
 */
static ph1_node_t* node_create (ph1_node_t* parent, uint16_t infix_length, uint16_t postfix_length, ph1_point_t* point, hypercube_address_t address)
{
	ph1_node_t* new_node = phtree_calloc (1, sizeof (*new_node));
 ...
 */
static ph1_node_t* node_try_add (ph1_node_t* node, bool* success, hypercube_address_t address, ph1_point_t* point)
{
	bool added = false;
 ...
 */
static ph1_node_t* node_insert_split (ph1_node_t* node, ph1_node_t* sub_node, ph1_point_t* point, int max_conflicting_bits)
{
	ph1_node_t* new_node = node_create (node, node->postfix_length - max_conflicting_bits, max_conflicting_bits - 1, point, 0);
 ...
 */
static ph1_node_t* node_handle_collision (bool* added, ph1_node_t* node, ph1_node_t* sub_node, ph1_point_t* point)
{
 ...
 */
static ph1_node_t* node_add (bool* added, ph1_node_t* node, ph1_point_t* point)
{
	hypercube_address_t address = calculate_hypercube_address (point, node);
	bool added_new_node = false;
	ph1_node_t* sub_node = node_try_add (node, &added_new_node, address, point);
...
static void entry_free (ph1_t* tree, ph1_entry_t* entry)
{
	if (entry->element)
	{
 ...
 */
void ph1_free_nodes (ph1_t* tree, ph1_node_t* node)
{
	if (!node)
	{
		return;
	...
	for (int iter = 0; iter < NODE_CHILD_COUNT; iter++)
	{
		if (node->children[iter])
		{
			...
			ph1_free_nodes (tree, node->children[iter]);
		}
	}
...
void ph1_clear (ph1_t* tree)
{
	...
	for (int iter = 0; iter < NODE_CHILD_COUNT; iter++)
	{
		...
		{
			ph1_free_nodes (tree, tree->root.children[iter]);
			tree->root.children[iter] = NULL;
		}
	}
...
static void for_each (ph1_t* tree, ph1_node_t* node, void (*function) (void* element, void* data), void* data)
{
	if (phtree_node_is_leaf (node))
	{
		for (int iter = 0; iter < NODE_CHILD_COUNT; iter++)
		{
			if (node->children[iter])
			{
				ph1_entry_t* entry = node->children[iter];
...
void* ph1_insert (ph1_t* tree, void* index)
{
	...
	ph1_entry_t* child = current_node->children[calculate_hypercube_address (&point, current_node)];
 ...
 */
ph1_entry_t* find_entry (ph1_t* tree, void* index)
{
	ph1_point_t point;
	tree->convert_to_point (tree, &point, index);
	ph1_node_t* current_node = &tree->root;
...
{
	ph1_entry_t* entry = find_entry (tree, index);
...
void ph1_remove (ph1_t* tree, void* index)
{
	...
	ph1_point_t point;
	tree->convert_to_point (tree, &point, index);
	ph1_node_t* current_node = entry->parent;
	...
	{
		ph1_node_t* parent = current_node->parent;
 ...
 */
static void node_query_window (ph1_node_t* node, ph1_window_query_t* query, void* data)
{
	...
	if (phtree_node_is_leaf (node))
	{
		for (int iter = 0; iter < NODE_CHILD_COUNT; iter++)
		{
			ph1_entry_t* child = node->children[iter];
 ...
 */
static void query_set_internal (ph1_t* tree, ph1_window_query_t* query, ph1_point_t* min, ph1_point_t* max, phtree_iteration_function_t function)
{
...
void ph1_query_set (ph1_t* tree, ph1_window_query_t* query, void* min_in, void* max_in, phtree_iteration_function_t function)
{
	if (!query)
	{
		return;
...
{
	ph1_window_query_t* new_query = phtree_calloc (1, sizeof (*new_query));
