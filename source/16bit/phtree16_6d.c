...
static void* add_child (ph6_t* tree, ph6_node_t* node, hypercube_address_t address)
{
	if (node->child_count >= node->child_capacity)
	{
		...
		node->children = tree->children_realloc (node->children, (node->child_capacity * sizeof (ph6_node_t)) + (sizeof (ph6_node_t) * 4));
...
{
	node->children = tree->children_malloc (4 * sizeof (ph6_node_t));
	node->child_capacity = 4;
	node->child_count = 0;
...
{
	ph6_t tree;
	ph6_initialize (&tree, element_create, element_destroy, children_malloc, children_realloc, children_free);
...
void ph6_query_box_set (ph6_t* tree, ph6_query_t* query, bool intersect, ph6_point_t* min_in, ph6_point_t* max_in, phtree_iteration_function_t function)
{
	if (!query)
	{
		return;
