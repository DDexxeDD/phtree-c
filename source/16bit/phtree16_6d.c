...
static void* add_child (ph6_node_t* node, hypercube_address_t address)
{
	if (node->child_count >= node->child_capacity)
	{
		...
		node->children = phtree_realloc (node->children, (node->child_capacity * sizeof (ph6_node_t)) + (sizeof (ph6_node_t) * 4));
 ...
 */
static void node_add_entry (ph6_node_t* node, ph6_point_t* point)
{
	...
	// 	create a new entry
	ph6_node_t* new_entry = add_child (node, address);

	new_entry->point = *point;
	new_entry->children = NULL;
}

static void node_initialize (ph6_node_t* node, uint16_t infix_length, uint16_t postfix_length, ph6_point_t* point)
{
	node->children = phtree_calloc (4, sizeof (ph6_node_t));
	node->child_capacity = 4;
	node->child_count = 0;
 ...
 */
static ph6_node_t* node_try_add (bool* added_new_node, ph6_node_t* node, hypercube_address_t address, ph6_point_t* point)
{
 ...
 */
static ph6_node_t* node_insert_split (ph6_node_t* parent, ph6_node_t* child, ph6_point_t* point, int max_conflicting_bits)
{
	/*
	...
	ph6_node_t* new_child = add_child (child, calculate_hypercube_address (&old_child.point, child));
 ...
 */
static ph6_node_t* node_handle_collision (ph6_node_t* node, ph6_node_t* sub_node, ph6_point_t* point)
{
	// if infix_length == 0
 ...
 */
static ph6_node_t* node_add (ph6_node_t* node, ph6_point_t* point)
{
	hypercube_address_t address = calculate_hypercube_address (point, node);
	...
	ph6_node_t* sub_node = node_try_add (&added_new_node, node, address, point);
...
{
	ph6_point_t empty_point = {{0, 0, 0, 0, 0, 0}};
...
{
	ph6_t tree;
	ph6_initialize (&tree, element_create, element_destroy, convert_to_key, convert_to_point, convert_to_box_point);
...
void* ph6_find (ph6_t* tree, void* index)
{
	ph6_point_t point;
	tree->convert_to_point (tree, &point, index);
	ph6_node_t* entry = ph6_find_entry (tree, &point);
...
void ph6_remove_child (ph6_node_t* node, hypercube_address_t address)
{
	int index = child_index (node, address);
...
void ph6_remove (ph6_t* tree, void* index)
{
	...
	if (current_node->child_count == 0)
	{
		...
		ph6_remove_child (parent, calculate_hypercube_address (&point, parent));
		stack_index--;
 ...
 */
static void query_set_internal (ph6_t* tree, ph6_query_t* query, ph6_point_t* min, ph6_point_t* max, phtree_iteration_function_t function)
{
...
{
	ph6_query_t query;
	ph6_query_set (tree, &query, min, max, function);

	return query;
}

void ph6_query_set (ph6_t* tree, ph6_query_t* query, void* min_in, void* max_in, phtree_iteration_function_t function)
{
	if (!query)
	{
		return;
...
void ph6_query_box_set (ph6_t* tree, ph6_query_t* query, bool intersect, void* min_in, void* max_in, phtree_iteration_function_t function)
{
	if (!query)
	{
		return;
...
{
	ph6_query_box_set (tree, query, true, point, point, function);
}
