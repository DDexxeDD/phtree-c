...
static void* add_child (ph3_node_t* node, hypercube_address_t address)
{
	if (node->child_count >= node->child_capacity)
	{
		...
		node->children = phtree_realloc (node->children, (node->child_capacity * sizeof (ph3_node_t)) + (sizeof (ph3_node_t) * 4));
...
static void node_add_entry (ph3_node_t* node, ph3_point_t* point)
{
	...
	// 	create a new entry
	ph3_node_t* new_entry = add_child (node, address);

	new_entry->point = *point;
	new_entry->children = NULL;
}

static void node_initialize (ph3_node_t* node, uint16_t infix_length, uint16_t postfix_length, ph3_point_t* point)
{
	node->children = phtree_calloc (4, sizeof (ph3_node_t));
	node->child_capacity = 4;
	node->child_count = 0;
 ...
 */
static ph3_node_t* node_try_add (bool* added_new_node, ph3_node_t* node, hypercube_address_t address, ph3_point_t* point)
{
 ...
 */
static ph3_node_t* node_insert_split (ph3_node_t* parent, ph3_node_t* child, ph3_point_t* point, int max_conflicting_bits)
{
	/*
	...
	ph3_node_t* new_child = add_child (child, calculate_hypercube_address (&old_child.point, child));
 ...
 */
static ph3_node_t* node_handle_collision (ph3_node_t* node, ph3_node_t* sub_node, ph3_point_t* point)
{
	// if infix_length == 0
 ...
 */
static ph3_node_t* node_add (ph3_node_t* node, ph3_point_t* point)
{
	hypercube_address_t address = calculate_hypercube_address (point, node);
	...
	ph3_node_t* sub_node = node_try_add (&added_new_node, node, address, point);
...
{
	ph3_t tree;
	ph3_initialize (&tree, element_create, element_destroy, convert_to_key, convert_to_point);
...
void* ph3_find (ph3_t* tree, void* index)
{
	ph3_point_t point;
	tree->convert_to_point (tree, &point, index);
	ph3_node_t* entry = ph3_find_entry (tree, &point);
...
void ph3_remove_child (ph3_node_t* node, hypercube_address_t address)
{
	int index = child_index (node, address);
