...
static bool node_in_window (ph3_dual_node_t* dual, ph3_query_t* window)
{
...
static bool entry_in_window (ph3_dual_node_t* dual, ph3_query_t* window)
{
 ...
 */
static hypercube_address_t calculate_hypercube_address (ph3_point_t* point, ph3_dual_node_t* dual)
{
...
static void* add_child (ph3_dual_node_t* dual, hypercube_address_t address)
{
	if (dual->node.child_count >= dual->node.child_capacity)
	{
		...
		dual->node.children = phtree_realloc (dual->node.children, (dual->node.child_capacity * sizeof (ph3_node_t)) + (sizeof (ph3_node_t) * 4));
	...
	memmove (dual->node.children + index + 1, dual->node.children + index, sizeof (ph3_node_t) * (dual->node.child_count - index));
	// zero the child we are adding
	memset (dual->node.children + index, 0, sizeof (ph3_node_t));
 ...
 */
static void node_add_entry (ph3_dual_node_t* dual, ph3_point_t* point)
{
	...
	// 	create a new entry
	ph3_entry_t* new_entry = add_child (dual, address);

	new_entry->point = *point;
	new_entry->element = NULL;
}

static void node_initialize (ph3_dual_node_t* dual, uint16_t infix_length, uint16_t postfix_length, ph3_point_t* point)
{
	dual->node.children = phtree_calloc (4, sizeof (ph3_node_t));
	dual->node.child_capacity = 4;
 ...
 */
static ph3_dual_node_t* node_try_add (bool* added_new_node, ph3_dual_node_t* dual, hypercube_address_t address, ph3_point_t* point)
{
	ph3_dual_node_t* node_out = NULL;
 ...
 */
static ph3_dual_node_t* node_insert_split (ph3_dual_node_t* parent, ph3_dual_node_t* child, ph3_point_t* point, int max_conflicting_bits)
{
	/*
	...
	ph3_dual_node_t old_child = *child;
	...
	ph3_dual_node_t* new_child = add_child (child, calculate_hypercube_address (&old_child.node.point, child));
 ...
 */
static ph3_dual_node_t* node_handle_collision (ph3_dual_node_t* dual, ph3_dual_node_t* sub_node, ph3_point_t* point)
{
	// if infix_length == 0
 ...
 */
static ph3_dual_node_t* node_add (ph3_dual_node_t* node, ph3_point_t* point)
{
	...
	ph3_dual_node_t* sub_node = node_try_add (&added_new_node, node, address, point);
...
static void entry_free (ph3_t* tree, ph3_dual_node_t* dual)
{
 ...
 */
static void free_nodes (ph3_t* tree, ph3_dual_node_t* dual)
{
	// this will free nodes recursively
	// 	worst case our stack is PHTREE_DEPTH deep
	void (*free_function) (ph3_t* tree, ph3_dual_node_t* node) = free_nodes;
 ...
 */
static void for_each (ph3_t* tree, ph3_dual_node_t* dual, void (*function) (void* element, void* data), void* data)
{
	if (phtree_node_is_leaf (dual))
	{
		for (int iter = 0; iter < dual->node.child_count; iter++)
		{
			ph3_entry_t* entry = (ph3_entry_t*) &dual->node.children[iter];
...
void* ph3_insert (ph3_t* tree, void* index)
{
	...
	ph3_dual_node_t* current_dual = &tree->root;
	...
	ph3_entry_t* entry = (ph3_entry_t*) (current_dual->node.children + offset);
 ...
 */
ph3_entry_t* ph3_find_entry (ph3_t* tree, ph3_point_t* point)
{
	ph3_dual_node_t* current_dual = &tree->root.node.children[child_index (&tree->root, calculate_hypercube_address (point, &tree->root))];
	...
	return (ph3_entry_t*) &current_dual->node.children[child_index (current_dual, address)];
}
...
void* ph3_find (ph3_t* tree, void* index)
{
	ph3_point_t point;
	tree->convert_to_point (tree, &point, index);
	ph3_entry_t* entry = ph3_find_entry (tree, &point);
...
void ph3_remove_child (ph3_dual_node_t* dual, hypercube_address_t address)
{
	int index = child_index (dual, address);
	ph3_dual_node_t* child = &dual->node.children[index];

	phtree_free (child->node.children);

	memmove (dual->node.children + index, dual->node.children + index + 1, sizeof (ph3_node_t) * (dual->node.child_count - index - 1));

	dual->node.child_count--;
	dual->node.active_children &= ~(PHTREE_CHILD_FLAG << (CHILD_SHIFT - address));
}

void ph3_remove_entry (ph3_t* tree, ph3_dual_node_t* dual, hypercube_address_t address)
{
	int index = child_index (dual, address);
	ph3_entry_t* entry = &dual->node.children[index].entry;
	...
	memmove (dual->node.children + index, dual->node.children + index + 1, sizeof (ph3_dual_node_t) * (dual->node.child_count - index - 1));
...
void ph3_remove (ph3_t* tree, void* index)
{
	...
	tree->convert_to_point (tree, &point, index);
	int stack_index = 0;
	ph3_dual_node_t* node_stack[PHTREE_DEPTH] = {0};
	ph3_dual_node_t* current_node = &tree->root;
	hypercube_address_t address;
	...
	if (current_node->node.child_count == 0)
	{
		...
		ph3_dual_node_t* parent = node_stack[stack_index];

		ph3_remove_child (parent, calculate_hypercube_address (&point, parent));
		stack_index--;
 ...
 */
static void node_query_window (ph3_dual_node_t* dual, ph3_query_t* query, void* data)
{
	...
	if (phtree_node_is_leaf (dual))
	{
		for (int iter = 0; iter < NODE_CHILD_MAX; iter++)
		{
			...
			{
				ph3_dual_node_t* child = &dual->node.children[child_index (dual, iter)];
 ...
 */
static void query_set_internal (ph3_t* tree, ph3_query_t* query, ph3_point_t* min, ph3_point_t* max, phtree_iteration_function_t function)
{
...
void ph3_query_set (ph3_t* tree, ph3_query_t* query, void* min_in, void* max_in, phtree_iteration_function_t function)
{
	if (!query)
	{
		return;
...
{
	ph3_query_t* new_query = phtree_calloc (1, sizeof (*new_query));
