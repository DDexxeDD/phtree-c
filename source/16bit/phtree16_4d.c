...
static bool node_in_window (ph4_dual_node_t* dual, ph4_query_t* window)
{
...
static bool entry_in_window (ph4_dual_node_t* dual, ph4_query_t* window)
{
 ...
 */
static hypercube_address_t calculate_hypercube_address (ph4_point_t* point, ph4_dual_node_t* dual)
{
...
static void* add_child (ph4_dual_node_t* dual, hypercube_address_t address)
{
	if (dual->node.child_count >= dual->node.child_capacity)
	{
		...
		dual->node.children = phtree_realloc (dual->node.children, (dual->node.child_capacity * sizeof (ph4_node_t)) + (sizeof (ph4_node_t) * 4));
	...
	memmove (dual->node.children + index + 1, dual->node.children + index, sizeof (ph4_node_t) * (dual->node.child_count - index));
	// zero the child we are adding
	memset (dual->node.children + index, 0, sizeof (ph4_node_t));
...
/*
 * insert a ph4_entry_t in a node
 */
static void node_add_entry (ph4_dual_node_t* dual, ph4_point_t* point)
{
	...
	// 	create a new entry
	ph4_entry_t* new_entry = add_child (dual, address);

	new_entry->point = *point;
	new_entry->element = NULL;
}

static void node_initialize (ph4_dual_node_t* dual, uint16_t infix_length, uint16_t postfix_length, ph4_point_t* point)
{
	dual->node.children = phtree_calloc (4, sizeof (ph4_node_t));
	dual->node.child_capacity = 4;
 ...
 */
static ph4_dual_node_t* node_try_add (bool* added_new_node, ph4_dual_node_t* dual, hypercube_address_t address, ph4_point_t* point)
{
	ph4_dual_node_t* node_out = NULL;
 ...
 */
static ph4_dual_node_t* node_insert_split (ph4_dual_node_t* parent, ph4_dual_node_t* child, ph4_point_t* point, int max_conflicting_bits)
{
	/*
	...
	ph4_dual_node_t old_child = *child;
	...
	ph4_dual_node_t* new_child = add_child (child, calculate_hypercube_address (&old_child.node.point, child));
 ...
 */
static ph4_dual_node_t* node_handle_collision (ph4_dual_node_t* dual, ph4_dual_node_t* sub_node, ph4_point_t* point)
{
	// if infix_length == 0
 ...
 */
static ph4_dual_node_t* node_add (ph4_dual_node_t* node, ph4_point_t* point)
{
	...
	ph4_dual_node_t* sub_node = node_try_add (&added_new_node, node, address, point);
...
static void entry_free (ph4_t* tree, ph4_dual_node_t* dual)
{
 ...
 */
static void free_nodes (ph4_t* tree, ph4_dual_node_t* dual)
{
	// this will free nodes recursively
	// 	worst case our stack is PHTREE_DEPTH deep
	void (*free_function) (ph4_t* tree, ph4_dual_node_t* node) = free_nodes;
 ...
 */
static void for_each (ph4_t* tree, ph4_dual_node_t* dual, void (*function) (void* element, void* data), void* data)
{
	if (phtree_node_is_leaf (dual))
	{
		for (int iter = 0; iter < dual->node.child_count; iter++)
		{
			ph4_entry_t* entry = (ph4_entry_t*) &dual->node.children[iter];
...
void* ph4_insert (ph4_t* tree, void* index)
{
	...
	ph4_dual_node_t* current_dual = &tree->root;
	...
	ph4_entry_t* entry = (ph4_entry_t*) (current_dual->node.children + offset);
 ...
 */
ph4_entry_t* ph4_find_entry (ph4_t* tree, ph4_point_t* point)
{
	ph4_dual_node_t* current_dual = &tree->root.node.children[child_index (&tree->root, calculate_hypercube_address (point, &tree->root))];
	...
	return (ph4_entry_t*) &current_dual->node.children[child_index (current_dual, address)];
}
...
void* ph4_find (ph4_t* tree, void* index)
{
	ph4_point_t point;
	tree->convert_to_point (tree, &point, index);
	ph4_entry_t* entry = ph4_find_entry (tree, &point);
...
void ph4_remove_child (ph4_dual_node_t* dual, hypercube_address_t address)
{
	int index = child_index (dual, address);
	ph4_dual_node_t* child = &dual->node.children[index];

	phtree_free (child->node.children);

	memmove (dual->node.children + index, dual->node.children + index + 1, sizeof (ph4_node_t) * (dual->node.child_count - index - 1));

	dual->node.child_count--;
	dual->node.active_children &= ~(PHTREE_CHILD_FLAG << (CHILD_SHIFT - address));
}

void ph4_remove_entry (ph4_t* tree, ph4_dual_node_t* dual, hypercube_address_t address)
{
	int index = child_index (dual, address);
	ph4_entry_t* entry = &dual->node.children[index].entry;
	...
	memmove (dual->node.children + index, dual->node.children + index + 1, sizeof (ph4_dual_node_t) * (dual->node.child_count - index - 1));
...
void ph4_remove (ph4_t* tree, void* index)
{
	...
	tree->convert_to_point (tree, &point, index);
	int stack_index = 0;
	ph4_dual_node_t* node_stack[PHTREE_DEPTH] = {0};
	ph4_dual_node_t* current_node = &tree->root;
	hypercube_address_t address;
	...
	if (current_node->node.child_count == 0)
	{
		...
		ph4_dual_node_t* parent = node_stack[stack_index];
 ...
 */
static void node_query_window (ph4_dual_node_t* dual, ph4_query_t* query, void* data)
{
	...
	if (phtree_node_is_leaf (dual))
	{
		for (int iter = 0; iter < NODE_CHILD_MAX; iter++)
		{
			...
			{
				ph4_dual_node_t* child = &dual->node.children[child_index (dual, iter)];
