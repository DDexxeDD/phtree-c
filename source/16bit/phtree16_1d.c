...
static void* add_child (ph1_node_t* node, hypercube_address_t address)
{
	if (node->child_count >= node->child_capacity)
	{
		...
		node->children = phtree_children_realloc (node->children, (node->child_capacity * sizeof (ph1_node_t)) + (sizeof (ph1_node_t) * 4));
...
{
	node->children = phtree_children_malloc (4 * sizeof (ph1_node_t));
	node->child_capacity = 4;
	node->child_count = 0;
