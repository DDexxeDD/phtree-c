...
phtree_key_t phtree_int{{bit_width}}_to_key (int{{bit_width}}_t a)
{
	...
	memcpy (&b, &a, sizeof (uint{{bit_width}}_t));
...
typedef struct {{prefix}}_node_t
{
	...
	// children are either {{prefix}}_node_t* or {{prefix}}_entry_t*
	// if the node is a leaf
	// 	children are {{prefix}}_entry_t*
	// if the node is _not_ a leaf
	// 	children are {{prefix}}_node_t*
	void* children[PHTREE_KEY_ONE << {{dimensions}}];
...
typedef struct {{prefix}}_window_query_t
{
	{{prefix}}_point_t min;
	{{prefix}}_point_t max;
	/*
...
static void point_subtract ({{prefix}}_point_t* point_a, unsigned int amount)
{
	for (int dimension = 0; dimension < DIMENSIONS; dimension++)
	{
...
static void point_add ({{prefix}}_point_t* point_a, unsigned int amount)
{
	for (int dimension = 0; dimension < DIMENSIONS; dimension++)
	{
		// prevent integer wrapping
...
{{#unused}}
static bool point_not_equal ({{prefix}}_point_t* point_a, {{prefix}}_point_t* point_b)
{
...
{{#unused}}
static bool point_in_window ({{prefix}}_point_t* point, {{prefix}}_window_query_t* window, int postfix_length)
{
	return (point_greater_equal (point, &window->min) && point_less_equal (point, &window->max));
}
{{/unused}}

static bool node_in_window ({{prefix}}_node_t* node, {{prefix}}_window_query_t* window)
{
...
static bool entry_in_window ({{prefix}}_entry_t* entry, {{prefix}}_window_query_t* window)
{
...
static void node_add_entry ({{prefix}}_node_t* node, {{prefix}}_point_t* point)
{
	...
	// 	create a new entry
	{{prefix}}_entry_t* new_entry = phtree_calloc (1, sizeof (*new_entry));
 ...
 * create a new node
 */
static {{prefix}}_node_t* node_create ({{prefix}}_node_t* parent, uint16_t infix_length, uint16_t postfix_length, {{prefix}}_point_t* point, hypercube_address_t address)
{
	{{prefix}}_node_t* new_node = phtree_calloc (1, sizeof (*new_node));
 ...
 */
static {{prefix}}_node_t* node_try_add ({{prefix}}_node_t* node, bool* success, hypercube_address_t address, {{prefix}}_point_t* point)
{
	bool added = false;
 ...
 */
static {{prefix}}_node_t* node_insert_split ({{prefix}}_node_t* node, {{prefix}}_node_t* sub_node, {{prefix}}_point_t* point, int max_conflicting_bits)
{
	{{prefix}}_node_t* new_node = node_create (node, node->postfix_length - max_conflicting_bits, max_conflicting_bits - 1, point, 0);
 ...
 */
static {{prefix}}_node_t* node_handle_collision (bool* added, {{prefix}}_node_t* node, {{prefix}}_node_t* sub_node, {{prefix}}_point_t* point)
{
 ...
 */
static {{prefix}}_node_t* node_add (bool* added, {{prefix}}_node_t* node, {{prefix}}_point_t* point)
{
	hypercube_address_t address = calculate_hypercube_address (point, node);
	bool added_new_node = false;
	{{prefix}}_node_t* sub_node = node_try_add (node, &added_new_node, address, point);
...
static void entry_free ({{prefix}}_t* tree, {{prefix}}_entry_t* entry)
{
	if (entry->element)
	{
	...
	void (*convert_to_point) ({{prefix}}_t* tree, {{prefix}}_point_t* out, void* input))
{
	...
	if ({{prefix}}_initialize (tree, element_create, element_destroy, convert_to_key, convert_to_point))
	{
		phtree_free (tree);
		return NULL;
	}
 ...
 */
void {{prefix}}_free_nodes ({{prefix}}_t* tree, {{prefix}}_node_t* node)
{
	if (!node)
	{
		return;
	...
	for (int iter = 0; iter < NODE_CHILD_COUNT; iter++)
	{
		...
		{
			// do this recursively
			// worst case our stack is {{bit_width}} deep
			{{prefix}}_free_nodes (tree, node->children[iter]);
		}
	}
...
void {{prefix}}_clear ({{prefix}}_t* tree)
{
	...
	for (int iter = 0; iter < NODE_CHILD_COUNT; iter++)
	{
		...
		{
			{{prefix}}_free_nodes (tree, tree->root.children[iter]);
			tree->root.children[iter] = NULL;
		}
	}
...
static void for_each ({{prefix}}_t* tree, {{prefix}}_node_t* node, void (*function) (void* element, void* data), void* data)
{
	if (phtree_node_is_leaf (node))
	{
		for (int iter = 0; iter < NODE_CHILD_COUNT; iter++)
		{
			if (node->children[iter])
			{
				{{prefix}}_entry_t* entry = node->children[iter];
...
void* {{prefix}}_insert ({{prefix}}_t* tree, void* index)
{
	...
	{{prefix}}_entry_t* child = current_node->children[calculate_hypercube_address (&point, current_node)];
 ...
 */
{{prefix}}_entry_t* find_entry ({{prefix}}_t* tree, void* index)
{
	{{prefix}}_point_t point;
	tree->convert_to_point (tree, &point, index);
	{{prefix}}_node_t* current_node = &tree->root;
...
void {{prefix}}_remove ({{prefix}}_t* tree, void* index)
{
	...
	{{prefix}}_point_t point;
	tree->convert_to_point (tree, &point, index);
	{{prefix}}_node_t* current_node = entry->parent;
	...
	{
		{{prefix}}_node_t* parent = current_node->parent;
 ...
 */
static void node_query_window ({{prefix}}_node_t* node, {{prefix}}_window_query_t* query, void* data)
{
	...
	if (phtree_node_is_leaf (node))
	{
		for (int iter = 0; iter < NODE_CHILD_COUNT; iter++)
		{
			{{prefix}}_entry_t* child = node->children[iter];
 ...
 */
static void query_set_internal ({{prefix}}_t* tree, {{prefix}}_window_query_t* query, {{prefix}}_point_t* min, {{prefix}}_point_t* max, phtree_iteration_function_t function)
{
...
void {{prefix}}_query_set ({{prefix}}_t* tree, {{prefix}}_window_query_t* query, void* min_in, void* max_in, phtree_iteration_function_t function)
{
	if (!query)
	{
		return;
...
{
	{{prefix}}_window_query_t* new_query = phtree_calloc (1, sizeof (*new_query));
